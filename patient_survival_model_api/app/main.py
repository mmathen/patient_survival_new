...
def update_metrics():
    ...
    test_feat = test.drop('DEATH_EVENT', axis=1).reset_index(drop=True)
    test_cnt = test['DEATH_EVENT'].values
    test_pred = make_prediction(input_data=test_feat)['predictions']
    r2 = round(r2_score(test_cnt, test_pred), 3)  # <-- Use built-in round()
    r2_metric.set(r2)
...
async def get_metrics():
    update_metrics()
    return Response(
        content=prom.generate_latest().decode("utf-8"),
